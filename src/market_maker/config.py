...
class MarketMakerSettings(BaseSettings):
    ...
    def field_groups(cls) -> Dict[str, FrozenSet[str]]:
        ...
        Used by the policy-file generator and advisor-agent validation
        to reason about parameter groups without hard-coding field names
        externally.
        """
    ...
    def field_metadata(cls) -> List[Dict[str, Any]]:
        ...
        group, and any ge/le/gt/lt/min/max constraints from the Field.
        Used by ``scripts/generate_policy.py``.
        """
        prefix = cls.model_config.get("env_prefix", "MM_")
        # Build reverse lookup: field_name → group_name
        ...
        for name, field_info in cls.model_fields.items():
            ...
            # Extract numeric constraints from pydantic metadata objects.
...
# ---------------------------------------------------------------------------
# Field group definitions (module-level to avoid pydantic metaclass issues)
# ---------------------------------------------------------------------------
